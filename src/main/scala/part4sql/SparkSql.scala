...
object SparkSql extends App {
  ...
  val spark = SparkSession.builder()
    ...
    .config("spark.sql.warehouse.dir", "src/main/resources/warehouse") // when we create a new db, it is added to this path
    .config("spark.sql.legacy.allowCreatingManagedTableUsingNonemptyLocation", "true") // read data from external db and overwrite the same existing data
    .getOrCreate()
  ...
  // use Spark SQL
  carsDF.createOrReplaceTempView("cars") // creating an alias equivalent to database
...
// 2
  spark.sql(
    """
      |select count(*)
      |from employees
      |where '1999-01-01' < hire_date and hire_date <= '2000-01-01'
      |""".stripMargin
  ).show()
